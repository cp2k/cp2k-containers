...
```
apptainer pull docker://cp2k/cp2k:2024.2_mpich_generic_cuda_P100_psmp
```

Run the container with the `--nv` flag using the `nvidia-smi` command from the container

```
apptainer run --nv ./cp2k_2024.2_mpich_generic_cuda_P100_psmp.sif nvidia-smi
```
...
```
apptainer run --nv ./cp2k_2024.2_mpich_generic_cuda_P100_psmp.sif nvcc --version
```

to retrieve the CUDA version of the actual container.  With the `run_tests` command

```
apptainer run --nv -B $PWD:/mnt ./cp2k_2024.2_mpich_generic_cuda_P100_psmp.sif run_tests
```
...
```
apptainer run -B $PWD cp2k_2024.2_mpich_generic_psmp.sif mpiexec -n 4 -genv OMP_NUM_THREADS=2 cp2k -i H2O-32.inp
```

with MPICH and similarly with OpenMPI

```
apptainer run -B $PWD cp2k_2024.2_openmpi_generic_psmp.sif mpiexec -n 4 -x OMP_NUM_THREADS=2 cp2k -i H2O-32.inp
```
...
mpiexec -n 4 apptainer run -B $PWD cp2k_2024.2_mpich_generic_psmp.sif cp2k -i H2O-32.inp
```
...
```
srun -n 4 apptainer run -B $PWD cp2k_2024.2_mpich_generic_psmp.sif cp2k -i H2O-32.inp
```
...
```
docker run -it --rm --shm-size=1g -v $PWD:/mnt -u $(id -u $USER):$(id -g $USER) cp2k/cp2k:2024.2_mpich_generic_psmp mpirun -n 4 -genv OMP_NUM_THREADS=2 cp2k -i H2O-32.inp
```

whereas the containers built with OpenMPI can be run, e.g. using

```
docker run -it --rm --shm-size=1g -v $PWD:/mnt -u $(id -u $USER):$(id -g $USER) cp2k/cp2k:2024.2_openmpi_generic_psmp mpirun -bind-to none -np 4 -x OMP_NUM_THREADS=2 cp2k -i H2O-32.inp
```
...
```
docker run -it --rm --gpus all cp2k/cp2k:2024.2_mpich_generic_cuda_P100_psmp nvidia-smi

docker run -it --rm --gpus all cp2k/cp2k:2024.2_mpich_generic_cuda_P100_psmp nvcc --version

docker run -it --rm --gpus all --shm-size=1g -v $PWD:/mnt -u $(id -u $USER):$(id -g $USER) cp2k/cp2k:2024.2_mpich_generic_cuda_P100_psmp run_tests
```
...
You can build your own CP2K docker container, e.g. using the Docker file `2024.2_mpich_generic_psmp.Dockerfile` for the CP2K version 2024.2, by running

```
docker build -f ./2024.2_mpich_generic_psmp.Dockerfile -t $USER/cp2k:2024.2_mpich_generic_psmp .
```
...
```
DOCKER_BUILDKIT=0 docker build -f ./2024.2_mpich_generic_psmp.Dockerfile -t cp2k/cp2k:2024.2_mpich_generic_psmp .
```

or to add the `--progress` flag when using the new docker builder `buildx`, e.g.

```
docker buildx build --progress=plain -f ./2024.2_mpich_generic_psmp.Dockerfile -t cp2k/cp2k:2024.2_mpich_generic_psmp .
```
