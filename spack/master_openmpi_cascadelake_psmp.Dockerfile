...
#
# Usage: podman build --shm-size=1g -f ./master_openmpi_cascadelake_psmp.Dockerfile -t cp2k/cp2k:master$(date +%Y%m%d)_openmpi_cascadelake_psmp .
...
RUN sed -e 's/require: target="\w*"/require: target="cascadelake"/' -e 's/- mpich/- openmpi/' -e '/^\s*- "mpich@/ s/^ /#/' -e '/^#\s*- "openmpi@/ s/^#/ /' -i /opt/cp2k/tools/spack/cp2k_deps_${CP2K_VERSION}.yaml && \
