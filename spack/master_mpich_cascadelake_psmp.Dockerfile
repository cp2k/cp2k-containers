...
#
# Usage: podman build --shm-size=1g -f ./master_mpich_cascadelake_psmp.Dockerfile -t cp2k/cp2k:master$(date +%Y%m%d)_mpich_cascadelake_psmp .
...
RUN sed -e 's/require: target="\w*"/require: target="cascadelake"/' -i /opt/cp2k/tools/spack/cp2k_deps_${CP2K_VERSION}.yaml && \
