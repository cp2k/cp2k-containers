...
#
# Usage: docker build -f ./2024.2_openmpi_generic_cuda_A100_psmp.Dockerfile -t cp2k/cp2k:2024.2_openmpi_generic_cuda_A100_psmp .
...
# Download CP2K
RUN git clone --recursive -b support/v2024.2 https://github.com/cp2k/cp2k.git /opt/cp2k
...
RUN printf "/opt/cp2k/tests/do_regtest.py --mpiexec \"mpiexec --bind-to none\" --maxtasks 8 --workbasedir /mnt \$* local_cuda psmp" \
