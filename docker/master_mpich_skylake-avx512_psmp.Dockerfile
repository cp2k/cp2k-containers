...
#
# Usage: docker build -f ./master_mpich_skylake-avx512_psmp.Dockerfile -t cp2k/cp2k:master$(date +%Y%m%d)_mpich_skylake-avx512_psmp .
...
RUN printf "/opt/cp2k/tests/do_regtest.py --maxtasks 8 --workbasedir /mnt \$* /opt/cp2k/exe/local psmp" \
