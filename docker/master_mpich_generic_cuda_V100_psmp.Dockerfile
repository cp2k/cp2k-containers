...
#
# Usage: docker build -f ./master_mpich_generic_cuda_V100_psmp.Dockerfile -t cp2k/cp2k:master$(date +%Y%m%d)_mpich_generic_cuda_V100_psmp .
...
RUN printf "/opt/cp2k/tests/do_regtest.py --maxtasks 8 --workbasedir /mnt \$* /opt/cp2k/exe/local_cuda psmp" \
